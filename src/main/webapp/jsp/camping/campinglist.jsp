...
<script type="text/javascript"src="//dapi.kakao.com/v2/maps/sdk.js?appkey=11cc932694fd38ffbe4f68b35d6b9427&libraries=services"></script>
</head>
...
<script>
		...
		geocoder.addressSearch('${param.cdetailregion}', function(result, status) {
			// 정상적으로 검색이 완료됐으면 
		...
		<c:forEach var="camping" items="${campings}" varStatus="status">
		geocoder.addressSearch('${camping.caddress}', function(result, status) {
			...
			var content = '<div class="mapWrap">'
				...
				+ '        <div class="title">'
				+ '            '
				+ "${camping.cname}" // JSTL 변수를 JavaScript 변수로 사용
				+ '        </div>'
				+ '        <div class="body">'
				+ '            <div class="img">'
				+ '                <img src="' + "${camping.cimage}" + '" width="73" height="70">'
				+ '           </div>'
				+ '            <div class="desc">'
				+ '                <div class="ellipsis">'
				+ "${camping.caddress}"
				+ '</div>'
				+ '                <div><a href="campingpage?no=${camping.no}&checkindate=${param.checkindate}&checkoutdate=${param.checkoutdate}" class="link">자세히 보기</a></div>'
				+ '            </div>' + '        </div>' + '    </div>'
				+ '</div>';
		...
		if(positions.length === ${campings.size()}){
	...
	<main id="product">
		...
		showSubMenu();
				...
				function getSelectedOption(selectId) {
					...
					selectedTextUrl = "http://localhost/campinglist?cdetailregion=" + encodeURIComponent(selectedText)+ "&checkindate=${formattedDate}&checkoutdate=${formattedDate}" + encodeURIComponent("");
		...
		/* 		 // 현재 URL에서 쿼리 문자열 가져오기
				    ...
				    var cdetailregionV = urlParams.get('cdetailregion');
				    
				if(cdetailregionV === '강남'){
			    ...
			    var checkoutdateV = urlParams.get('checkoutdate');
				   ...
				   var selectedText = localStorage.getItem("selectedText"); // selectedText  지역 값을 가져옴
					 ...
					 var cdetailregionV = urlParams.get('cdetailregion');
				         ...
				         var updatedURL = "http://localhost/campinglist?cdetailregion=" + encodeURIComponent(cdetailregionV) + "&checkindate=" + encodeURIComponent(checkindate) + "&checkoutdate=" + encodeURIComponent(checkoutdate); 
				        // 페이지 새로고침
						...
						<!-- 초기화, 적용 버튼  -->
						<div class="btn_wrap">
							<button type="button" onclick="location.href='Campinglist?cdetailregion=강남&checkindate=${formattedDate}&checkoutdate=${formattedDate}'">초기화</button>
							<button type="button" onclick="comfort()">적용</button>
						</div>
						<br>

						<section>
							<br> <strong>캠핑 유형</strong><br> <br>
							<ul>
								<li><input type="checkbox" id="grade_0" name="grade[]"
									class="inp_chk" value="오토캠핑" /> <label for="grade_0"
									class="label_chk">오토캠핑</label></li>
								<li><input type="checkbox" id="grade_1" name="grade[]"
									class="inp_chk" value="글램핑" /> <label for="grade_1"
									class="label_chk">글램핑</label></li>
								<li><input type="checkbox" id="grade_2" name="grade[]"
									class="inp_chk" value="카라반" /> <label for="grade_2"
									class="label_chk">카라반</label></li>
							</ul>
						</section>
						<br>
...
<!--   필요없어서 우선 주석처리... 
						...
						<section>
							...
							<ul class="hide_type half">
								...
								<li><input type="checkbox" id="tmino_1" name="tmino[]"
									class="inp_chk" value="편의점" /> <label for="tmino_1"
									class="label_chk">편의점</label></li>
								<li><input type="checkbox" id="tmino_2" name="tmino[]"
									class="inp_chk" value="취사" /> <label for="tmino_2"
									class="label_chk">취사</label></li>
								<li><input type="checkbox" id="tmino_3" name="tmino[]"
									class="inp_chk" value="화장실" /> <label for="tmino_3"
									class="label_chk">화장실</label></li>
							</ul>
						</section>
						<br>
						...
						<section>
							...
							<ul class="hide_type half">
								<li><input type="checkbox" id="tmino_24" name="tmino[]"
									class="inp_chk" value="샤워실" /> <label for="tmino_24"
									class="label_chk">샤워실</label></li>
								<li><input type="checkbox" id="tmino_25" name="tmino[]"
									class="inp_chk" value="와이파이" /> <label for="tmino_25"
						...
						<section>
							...
							<ul class="hide_type half">
								<li><input type="checkbox" id="tmino_36" name="tmino[]"
									class="inp_chk" value="장비대여" /> <label for="tmino_36"
									class="label_chk">장비대여</label></li>
						      ...
						      /*   var values2 = document.getElementsByName("bedtype[]");
						        ...
						        for (var i = 0; i < values3.length; i++) {
						         ...
						         /* 선택한 날짜 값을 URL 파라미터로 추가하고 페이지 새로고침 */
						            ...
						            addDataToURL(ctype, ccomfort, cpeople);
						         
							       
						          var cpeople = document.querySelector('.cnt_people span').textContent; // 인원 수 값을 가져옴    
						     	  var selectedText = localStorage.getItem("selectedText"); // selectedText  지역 값을 가져옴
						   	...
						   	<!--  선택한 날짜 값을 URL 파라미터로 추가하고 페이지 새로고침하는 함수 -->
						     	    function addDataToURL(ctype, ccomfort, cpeople) {
						     		  ...
						     		  // 현재 페이지 URL 가져오기
							 	       ...
							 	       ctype = ctype.map(function(value) {
							 	            return value.trim();
							 	        });
							 	   ...
							 	   /*      hbedtype = hbedtype.map(function(value) {
							 	        ...
							 	        ccomfort = ccomfort.map(function(value) {
							 	            return value.trim();
							 	        });
							 	       
							 	        
							 	       // 각 변수가 빈 문자열이 아니면 encodeURIComponent 적용
							 	        var ctypeParam = ctype.length > 0 ? encodeURIComponent(ctype.join(',')) : "";
							 	       /*  var cbedtypeParam = cbedtype.length > 0 ? encodeURIComponent(cbedtype.join(',')) : ""; */
							 	        var ccomfortParam = ccomfort.length > 0 ? encodeURIComponent(ccomfort.join(',')) : "";
							 	        var cpeopleParam = cpeople !== null && cpeople !== "" ? encodeURIComponent(cpeople) : "";

							 	        // 각 파라미터에 대한 값을 할당하여 URL에 추가
							 	        updatedURL += "&ctype=" + ctypeParam;
							 	       /*  updatedURL += "&cbedtype=" + cbedtypeParam; */
							 	        updatedURL += "&ccomfort=" + ccomfortParam;
							 	        updatedURL += "&cpeople=" + cpeopleParam;
			...
			<div class="right">
				...
				<c:choose>
					<c:when test="${not empty campings}">

						<c:forEach var="camping" items="${campings}">

							<a href="campingpage?no=${camping.no}&checkindate=${param.checkindate}&checkoutdate=${param.checkoutdate} " class="thumb">
								<table>
									<tr>
										<td width="300px" height="250px"><img
											src="${camping.cimage} " alt="호텔이미지" width="250px"
											height="230px"></td>

										<td>
											<h2 class="name">${camping.cname}</h2>
											<p>${camping.cdetailregion }</p> 
											<!-- <br> <span>*****</span> -->
					...
					<c:otherwise>
						...
						<p>No camping information available.</p>
					</c:otherwise>
				</c:choose>
